    ...
    def process(cls, element):
        ...
        if element.tag == 'dl':
            ...
            for child in element:
                data_type = child.attrib[
                    u'data-type'] if u'data-type' in child.attrib else u''
                el = Run()
                ...
                if data_type == u'term':
                    me.term = el
                elif data_type == u'meaning':
                    me.meaning = el
                else:
                    logger.warn(u'Unhandled data type %s', data_type)
        return me
    ...
    def process(cls, element):
        ...
        for child in element:
            if child.tag == u'h2':
                continue
            data_type = child.attrib[
                u'data-type'].strip() if u'data-type' in child.attrib else u''
            glossary_class = child.attrib[
                u'class'].strip() if u'class' in child.attrib else u''
