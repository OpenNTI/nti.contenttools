...
class Section(types.Section):
    ...
    @classmethod
    def process(cls, element):
        data_depth = element.attrib[
            'data-depth'] if u'data-depth' in element.attrib else None

        if data_depth == u'1':
            me = types.SubSection()
        elif data_depth == u'2':
            me = types.SubSubSection()
        elif data_depth == u'3':
        ...
        if u'class' in element.attrib:
            me.section_type = element.attrib['class']
    ...
    def process(cls, element, epub, type_=None):
        ...
        for child in element:
            if child.tag == 'dt':
                ...
                if me.children[count_child].desc is None:
                    desc = []
    ...
    def process(cls, element):
        ...
        if 'data-number-style' in element.attrib:
            ...
            if numbering_type == u'1':
                ...
                logger.warn(
                    "UNHANDLED OrderedList numbering format type %s",
                    numbering_type)
    ...
    def process(cls, element):
        ...
        if 'style' in element.attrib:
            ...
            if u'circle' in numbering_style:
                ...
                logger.warn(
                    "UNHANDLED UnorderedList numbering format type %s",
                    numbering_style)
        for child in element:
            el = None
            if child.tag == 'li':
...
def _process_div_elements(element, parent):
    ...
    if type_ is None:
        ...
        if type_ == u'document-title':
            ...
            data_label = element.attrib[
                u'data-label'] if u'data-label' in element.attrib else None
            if data_label in [u'Click and Explore']:
                ...
                if el.caption is None or el.caption.isspace() or len(el.caption) == 0:
                    el.caption = data_label
            else:
                el = Run()
            ...
            list_type = element.attrib[
                'data-list-type'] if 'data-list-type' in element.attrib else None
            el = OrderedList() if list_type == 'enumerated' else UnorderedList()
            el.add_child(Run.process(element))
        elif type_ == u'item':
            el = Item.process(element)
        else:
            if type_ not in [u'newline', u'equation', u'commentary', u'']:
                logger.warn('process div as default %s', type_)
            el = Run.process(element)
    return el
...
class Iframe(types.Iframe):
    ...
    def process(cls, element):
        ...
        if 'id' in element.attrib:
            me.label = element.attrib['id']
        if u'src' in element.attrib:
            me.source = element.attrib[u'src']
        me = check_element_text(me, element)
        me = check_child(me, element)
        me = check_element_tail(me, element)
        return me
