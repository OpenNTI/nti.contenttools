...
class DocxFile(object):
    ...
    def __init__(self, filename, image_dir='Images'):
        ...
        self.relationships = ElementTree.fromstring(
            self.zip.read('word/_rels/document.xml.rels'))
        try:
            self.footnotes = ElementTree.fromstring(self.zip.read('word/footnotes.xml'))
        except KeyError:
            self.footnotes = None
        try:
            self.footnote_relationships = ElementTree.fromstring(
                self.zip.read('word/_rels/footnotes.xml.rels'))
        except KeyError:
            ...
            try:
                ...
                self.endnotes = None
            try:
                self.endnote_relationships = ElementTree.fromstring(
                    self.zip.read('word/_rels/endnotes.xml.rels'))
            except KeyError:
                self.endnote_relationships = ElementTree.fromstring(DEFAULT_RELATIONSHIPS)
        try:
            a, n = process_numbering(
                ElementTree.fromstring(
                    self.zip.read('word/numbering.xml')))
        ...
        # The numbering_collection attribute is used to determine what the next
        # list item's number is
        self.numbering_collection = {}
        self.tagparser = NTITagParser()
        # This call needs to be last, because we rely on other parts of this
        # object to be initialized first
