				...
				if class_ in ['title']:
					...
					for sub_el in child:
						if sub_el.tag == 'div' and sub_el.attrib['class'] == 'mediaobject':
							...
							_, link, note = process_link_interactive(sub_el, epub)
							notes = notes + note
							me.set_notes(notes)
							if link is not None:
								me.set_link(link)
	...
	for child in element:
		if isinstance(child, types.Hyperlink):
			...
			if isinstance(child.children[0], types.Run):
				_, note = process_run_interactive(child, epub)
				notes = notes + note
			else:
...
def _process_div_elements( element, epub ):
	...
	if class_ in ['note interactive']:
	...
	elif class_ in ['cnx-eoc summary']:
		el = Run()
		num_child = 0
		for _ in element.getchildren():
			if num_child == 0 :
		...
		pass
	elif class_ in ['cnx-eoc art-exercise', 'cnx-eoc multiple-choice', 'cnx-eoc free-response']:
